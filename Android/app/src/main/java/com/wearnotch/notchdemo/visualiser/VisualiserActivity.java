...
public class VisualiserActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    public static Intent createIntent(Context context, @RawRes int rawExample) {
        Intent i = new Intent(context, VisualiserActivity.class);
        i.putExtra(PARAM_INPUT_RAW, rawExample);
        return i;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRawDemo = getIntent().getIntExtra(PARAM_INPUT_RAW, -1);
        ...
        new AsyncTask<Void, Void, ExtendedRenderer>() {
                ...
                try {
                    ...
                    if (mZipUri != null) {
                        for (Parcelable uri : mZipUri) {
                            ...
                            dataIndex = renderer.addRendererContext(renderer.createRendererContext(data));
                        }
                    } else if (mData != null) {
                        dataIndex = renderer.addRendererContext(renderer.createRendererContext(mData));
                    } else if (mRawDemo != -1) {
                        VisualiserData data = VisualiserData.fromStream(getResources().openRawResource(mRawDemo));
                        dataIndex = renderer.addRendererContext(renderer.createRendererContext(data));
                    }
            ...
            protected void onPostExecute(ExtendedRenderer renderer) {
                ...
                if (renderer != null) {
                    ...
                    mRenderer.setHighlightBones(mHighlightBones);
                    mRenderer.setRootMovement(!mPinToCentre);
                    // Enable lateral movement if needed
                    // mRenderer.setLateralMovement(flag);
    ...
    class ExtendedRenderer extends NotchSkeletonRenderer {
        ...
        public RendererContext createRendererContext(VisualiserData data, String obj, String mtl, RigidBody.StickCustomizer stick) {
            ...
            if (mRealTime) {
                int defColor1 = Color.argb(255, 0, 158, 0);
                int defColor2 = Color.argb(255, 255, 100, 255);

                final PlotDemo plotDemo = new PlotDemo(new HashMap<>(), data.getSkeleton(), isGroundDrawn, defColor1, defColor2);
                if (!isGroundDrawn) isGroundDrawn = true;
                plotDemo.init();

                return new RendererContext(data, obj, mtl, null) {
                    ...
                    public void draw() {
                        ...
                        plotDemo.setSimpleDataMap(realTimePlotData);
        ...
        protected void onFrameIndexChanged(int dataIndex, int frameIndex) {
            ...
            if (dataIndex == 0 && mRealTime) {
                mergeRealTimePlotData();
            }
        }
